  ...
  if($pay=='credit'){
    ...
    $srcid=$pay;
      if($srcid==200016){ //Cash in Hands
        ...
        if(!empty($discount)){
          ...
          $data=mysqli_query($con,"INSERT INTO journal (jid,actid,desp,type,typeid,ref,balance,cr,datec,dateup)VALUES ('$jid','$discid','$desp','$disctype','$disctypeid',1,'$discbalance','$discount','$datec','$dateup')")or die( mysqli_error($con) );

          $desp='Discount Recieved from '.$actname;

          $data=mysqli_query($con,"INSERT INTO journal (jid,actid,desp,type,typeid,ref,balance,cr,datec,dateup)VALUES ('$jid','$act','$desp','$acttype','$acttypeid',1,'$actbalance','$discount','$datec','$dateup')")or die( mysqli_error($con) );
        }
    ...
    for ($i=0; $i < $x; $i++) {
      ...
      $sqls = "UPDATE items SET `stock` = '$stock',`price` = '$price',`sellprice` = '$pprice' WHERE `id` = $pid"  ;
      mysqli_query($con, $sqls)or die(mysqli_error($con));
